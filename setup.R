...
## simdat <- function(mcr.params) {
##    mkDf(nsubj = mcr.params[["nsubj"]],
##         nitem = mcr.params[["nitem"]],
##         mcr.params = mcr.params[-which(names(mcr.params) %in% c("nsubj", "nitem"))])
##}
...
simdat2 <- function(mcr.params) {
    ...
    exp.list$Cond <- c(-.5, .5)
    subj <- cbind(SubjID = factor(1:nsubj), MASS::mvrnorm(nsubj, 
                      mu = c(0, 0),
                      Sigma = mkSigma(mcr.params[["t00"]], mcr.params[["t11"]], 
                          mcr.params[["rsub"]])))
    colnames(subj) <- c("SubjID", "ri.subj", "rs.subj")
    item <- cbind(ItemID = factor(1:nitem), MASS::mvrnorm(nitem, 
                      mu = c(0, 0), Sigma = mkSigma(mcr.params[["w00"]], mcr.params[["w11"]], 
                                        mcr.params[["ritm"]])))
...
parms_fixed <- list(
    ...
    t00 = 100^2, # random intercept
    w00 = 100^2) # random slope

n_increments = 7L ## 
## n_increments = 10000L ## recreate entire set of simulations
parms_vary <- cbind(t11 = seq(0, 120, length.out = n_increments)^2, # by-subj slope
                    rsub = .6, 
